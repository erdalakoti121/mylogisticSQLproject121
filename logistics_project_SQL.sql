...
-- Q7 warehouse contributing highest logistics cost.alter
...
select * from (
select warehouse_city, transport_cost, row_number() over(partition by warehouse_city) as max_cost from shipments_data) t 
where max_cost <= 3;
...
sum (fuel_cost) / (select sum(fuel_cost) from shipments_data)
from shipments_data
group by warehouse_city;
